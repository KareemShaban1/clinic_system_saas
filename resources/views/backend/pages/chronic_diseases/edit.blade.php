    ...
    <div class="col-md-12 mb-30">
        <div class="card card-statistics h-100">
            <div class="card-body">
                        ...
                        <table class="table table-bordered table-responsive" id="table">
                            ...
                            <tbody id="tbody">
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" name="title[]" value="{{old('title',$chronic_disease->title)}}" class="form-control" placeholder="{{trans('chronic_diseases_trans.Disease_Name')}}">
                                        @error('title')
                                        <p class="alert alert-danger">{{ $message }}</p>
                                        @enderror 
                                    </td>
                                    <td>
                                        <input type="text" name="measure[]"  value="{{old('title',$chronic_disease->measure)}}"class="form-control" placeholder="{{trans('chronic_diseases_trans.Disease_Measure')}}">
                                        @error('measure')
                                        <p class="alert alert-danger">{{ $message }}</p>
                                        @enderror 
                                    </td>
                                    <td>
                                        <input id="datepicker-action" data-date-format="yyyy-mm-dd" name="date[]"  value="{{old('title',$chronic_disease->date)}}" class="form-control" placeholder="{{trans('chronic_diseases_trans.Disease_Date')}}">
                                        @error('date')
                                        <p class="alert alert-danger">{{ $message }}</p>
                                        @enderror 
                                    </td>
                                    <td>
                                        <input type="text" name="notes[]" class="form-control" value="{{old('title',$chronic_disease->notes)}}" placeholder="{{trans('chronic_diseases_trans.Notes')}}">
                                        @error('notes')
                                        <p class="alert alert-danger">{{ $message }}</p>
                                        @enderror 
                                    </td>
             ...
             <button type="submit" class="btn btn-primary">{{trans('chronic_diseases_trans.Edit')}}</button>
...
<script>
    ...
    $('thead').on('click', '.addRow', function() {
        var tr = '<tr>' +
            '<td><input class="form-control" hidden value="{{$chronic_disease->reservation_id}}"  name="reservation_id[]" type="text" ></td>'+
            '<td><input class="form-control" hidden value="{{$chronic_disease->patient_id}}"  name="patient_id[]" type="text" ></td>'+
