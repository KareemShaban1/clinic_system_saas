        ...
        <div class="card card-statistics h-100">
            <div class="card-body">
                ...
                <form method="post" enctype="multipart/form-data"
                    action="{{ Route('backend.reservations.update', $reservation->reservation_id) }}"
                    autocomplete="off">
                    @csrf
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="res_num" class="custom-select mr-sm-2">
                                        @for ($i = 1; $i <= $numberOfRes; $i++)
                                            <option value="{{ $i }}"
                                                {{ $reservationResNum == $i ? 'selected style=background:gainsboro' : ($i == $reservation->res_num ? 'selected' : '') }}>
                                                {{ $i }}
                                            </option>
                                        @endfor
                                    </select>
